...
@section('content')
    <div class="container mx-auto px-4 py-8">
        ...
        <table class="table-auto w-full border-collapse border border-spacing-2">
            <thead>
                <tr>
                    <th class="border p-2 bg-blue-500 text-black">#</th>
            ...
            <tbody>
                @foreach($barang as $item)
                    <tr>
                        <td class="border p-2 text-black">{{ $loop->iteration }}</td>
                        <td class="border p-2 text-black">{{ $item->nama_produk }}</td>
                        <td class="border p-2 text-black">{{ $item->kode_produk }}</td>
                        <td class="border p-2 text-black">{{ $item->harga_produk }}</td>
                        <td class="border p-2 text-black">{{ $item->stok }}</td>
                        <td class="border p-2 text-black">{{ $item->deskripsi }}</td>
